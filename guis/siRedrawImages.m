function siRedrawImages(channelList, frame)
    disp('siRedrawImages bypassed - necessary?');
return
	...
	for channel=channelList
		if nargin<2
			set(state.internal.imagehandle(channel), 'CData', lastAcquiredFrame{channel}(:,:), ...
				'YData', [1 state.acq.linesPerFrame]);
            disp('siRedrawImages have to deal with EraseMode'); %'EraseMode', 'none', 
		else
			frame=min(size(imageData{channel},3), frame);
			set(state.internal.imagehandle(channel), 'CData', imageData{channel}(:,:,frame), ...
				'YData', [1 state.acq.linesPerFrame]);
            %'EraseMode', 'none', 
		end
	end
	...
	if state.internal.composite			
		...
		set(state.internal.compositeImagehandle, 'CData', ...
