function siFigures_redraw(channelList, frame)

	global state imageData compositeData lastAcquiredFrame
	...
	for channel=channelList
		if nargin<2
            frameData=lastAcquiredFrame{channel};
        else
            frameData=imageData{channel}(:,:,min(frame, size(imageData{channel},3)));
        end
        
        for stripe=0:(state.internal.numberOfStripes-1)
            startLine = 1 + state.acq.linesPerFrame/state.internal.numberOfStripes*stripe;
            stopLine = startLine + state.acq.linesPerFrame/state.internal.numberOfStripes - 1;
            state.imaging.internal.imageHandle{channel, stripe+1}.CData = ...
                    frameData(startLine:stopLine,:);
        end
	end
...
% 		set(state.imaging.internal.compositeImagehandle, 'CData', ...
