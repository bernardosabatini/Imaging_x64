function varargout=settingsManager(varargin)
	...
	persistent setValues setNames setVariables savePaths saveFiles
	if ~iscell(setValues)
		setValues={};
		setNames={};
		setVariables={};
	...
	if action==1 % initialize
        if ~iscell(varargin{3}) % avoid non cell var inputs
            disp('settingsManager : Input string needs to be a cell aray'); return
        end
		...
		savePaths{setPosition}='';
		saveFiles{setPosition}='';
		savePosition=1;
		...
		if action>=10 && action<40	% the lower actions work on entire sets and do not need a position arguments
    ...
    elseif any(action==[1 10 11]) % actions that store information
        for counter=pickedVariable
            try
                ...
                if strcmp(strtok(exception1.message,' '),'Undefined') % Check existance of variable
                    disp('settingsManager : Input variable needs to be defined'); return
                elseif strcmp(strtok(exception1.message,' '),'Cell') % Check if input var is cell
                    disp('settingsManager : Input string needs to be a cell aray'); return
                end
            end
        end
    ...
    elseif action==40 % Save Data Set
        switch size(varargin,2)
            ...
            case 3
                pname=varargin{3};
            case 4
                pname=varargin{3}; fname=varargin{4};
        end
            
        [pname,fname]=saveDataSets({setNames{setPosition}, setVariables{setPosition}, setValues{setPosition}},pname,fname);
        
        savePaths{setPosition}=pname;
        saveFiles{setPosition}=fname;
        
    elseif action==41 % Load Data Set
        switch size(varargin,2)
            case 2
                pname=''; fname='';
            case 3
        ...
        DataSet = loadDataSets(savePaths{setPosition},saveFiles{setPosition});
        setNames{setPosition}=DataSet{1};
        setVariables{setPosition}=DataSet{2};
        setValues{setPosition}=DataSet{3};
        savePaths{setPosition}=DataSet{4};
        saveFiles{setPosition}=DataSet{5};
