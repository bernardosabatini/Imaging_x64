function siSession_allocateMemory
	...
	% Cold Spring Harbor Labs, Harvard Medical School
	% 2001-2014

	if state.acq.dualLaserMode==1   % lasers go simulataneously therefor one image window per laser
		channelList=1:state.imaging.daq.maximumNumberOfInputChannels;
	else
	...
	for channelCounter = channelList
		...
		if state.acq.acquiringChannel(inputChannelCounter) % acquiring the channel
