function makeNewPcellBinaryOutput
	...
	lineDelayFraction = state.acq.lineDelay/state.acq.msPerLine; % calculate fractional line delay
	pOut = zeros(state.internal.lengthOfXData ,1);					% Fill with zero for the flyback
	if state.acq.bidi
		state.internal.flybackDecimal = 1-state.acq.fillFraction-lineDelayFraction;
		pStart =  1+round(state.internal.lengthOfXData * ...
			((lineDelayFraction/2+state.acq.mirrorLag-state.pcell.pcellDelay)/state.acq.msPerLine));
